...
(function() {
    ...
    async function loadPage(page, saveToHistory = true) {
        ...
        // Для страницы ЗАКАЗОВ — используем iframe
        if (page === 'orders') {
            if (contentArea) {
        ...
        // Для страницы ДОСТАВКИ — заглушка (можно будет добавить позже)
        if (page === 'delivery') {
            if (contentArea) {
                contentArea.innerHTML = `
                    <div class="content-card" style="display: flex; align-items: center; justify-content: center; min-height: 400px; background: white; border-radius: 20px;">
                        <div style="text-align: center; color: #999;">
                            ...
                            <div>Страница "Доставка" в разработке</div>
                            <div style="margin-top: 16px; font-size: 12px;">Настройка способов доставки будет добавлена позже</div>
                        </div>
                    </div>
                `;
            }
        ...
        // Для страницы НАСТРОЕК — заглушка
        if (page === 'settings') {
            if (contentArea) {
                contentArea.innerHTML = `
                    <div class="content-card" style="display: flex; align-items: center; justify-content: center; min-height: 400px; background: white; border-radius: 20px;">
    ...
    const validPages = ['products', 'chat', 'widget-editor', 'orders'];
